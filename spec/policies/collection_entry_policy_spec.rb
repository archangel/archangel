...
RSpec.describe CollectionEntryPolicy, type: :policy do
  ...
  context 'with User under the `admin` role' do
    ...
    it { is_expected.to permit_actions(%i[index show new create edit update destroy history reposition restore]) }
  end

  context 'with User under the `editor` role' do
    before { create(:user_site, :editor_role, user: user, site: site) }

    it { is_expected.to permit_actions(%i[index show edit update reposition]) }
  ...
  context 'with User under the `reader` role' do
    ...
    it { is_expected.to forbid_actions(%i[new create edit update destroy history reposition restore]) }
  end
end
